...
const App = () => {
  ...
  const fetchTeams = () => {
    fetch("http://localhost:3000/")
      ...
      .then((data) => {
        setTeams([...data.teams]); // should log the JSON data from the server
      })
      .catch((error) => {
        console.error(error); // handle any errors from the request
      });
  };
      ...
      <div className="container mx-auto p-6">
        <div className="flex flex-row items-baseline justify-between">
          <h2>UC App Health</h2>
          {
            <Menu
              as="div"
              className="relative inline-block pl-6 text-left lg:hidden"
            >
              <p>
                    ...
                    <Menu.Item>
                      {({ active }) => (
                        <button
                          ...
                          className={`${active && "bg-slate-100 "} menuItem`}
                        >
                          {team.name}
                          {teamstats[team.name] &&
                            teamstats[team.name].FAILURE > 0 && (
                              <span className="mx-2 max-w-min rounded-full bg-red-400 px-2 text-white">
                                {teamstats[team.name].FAILURE}
                              </span>
                            )}
                        </button>
                      )}
                ...
                <Menu.Item>
                  <button className="menuItem">
                    <a href="https://github.com/ITDeptUtahCountyGovernment/IT-aws-healthcheck-monitor-client/issues">
                      Report any issues{" "}
                      <BsGithub className="inline" aria-hidden="true" />
                    </a>
                  </button>
                </Menu.Item>
                <Menu.Item>
        ...
        <div className="flex w-full flex-row justify-between">
          ...
          <div className="flex w-full flex-col lg:w-3/4">
            <div className="mb-2 flex w-full items-baseline justify-between">
              <h1 className="inline">
                {" "}
                {teamID === -1 ? "System Log" : TEAMS[teamID].name}{" "}
              </h1>
            </div>
            {!(teamID === -1) && (
              <p className="mb-8">
                {TEAMS[teamID].lead} (x{TEAMS[teamID].ext}){" "}
            ...
            {teamID === -1 ? (
              ...
              <StatusCollection data={data} team={TEAMS[teamID].name} />
            )}
          </div>
        </div>
      </div>
