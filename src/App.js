...
const App = () => {
  ...
  const getDetailPanel = () => {
      ...
      return (
        <div className="flex h-80v w-full  flex-col">
          <div className="scrollbar-hide block w-full overflow-scroll rounded-lg border border-slate-800">
            <StatusCollection
              data={getFilteredData(searchKey)}
              showTeam={true}
            />
          </div>
        </div>
    ...
    if (currentNav === "System Log") {
      return (
        <div className="flex w-full flex-col">
          <Syslog syslog={syslog} syslogerr={syslogerr} url={URL} />
        </div>
      );
    }
    ...
    if (teams.some((team) => team.name === currentNav)) {
      return (
        <div className="flex h-full w-full  flex-col space-y-6 lg:space-y-10">
          {teamstats[currentNav] && (
            <DonutCharts teamstats={teamstats[currentNav]} />
          )}
          <div className="scrollbar-hide block w-full overflow-scroll rounded-lg border border-slate-800">
            <StatusCollection
              data={data.filter((entry) => entry.name === currentNav)}
            />
          </div>
        </div>
      );
        ...
        >
          <h2>
            ...
            {teams.some((team) => team.name === getCurrentNav()) ? (
              <ContactPopover
                team={teams.find((team) => team.name === getCurrentNav())}
              />
            ) : (
              getCurrentNav()
            )}
          ...
          >
            <BiSearch
              onClick={() =>
                searchInput ? setSearchInput(null) : setSearchInput(" ")
              }
