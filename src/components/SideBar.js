      ...
      <a href="/">
        <li
          className={`sidebarTab rounded-lg ${
            !(props.currentNav === 'System Log') ? 'bg-slate-700 text-gray-200' : ''
          }`}
        >
          <h3>
        ...
        <ul className="border-l">
          {props.teams.map((team, index) => (
            <a href={`/${team.name.replace(/\s+/g, '-')}`}>
              ...
              >
                {team.name}
                {props.teamstats[team.name] && props.teamstats[team.name].GOOD > 0 && (
                  <span className="bg-good ml-2 max-w-min rounded-full px-2 font-bold text-black ">
                    {props.teamstats[team.name].GOOD}
                  </span>
                )}
                {props.teamstats[team.name] && props.teamstats[team.name].WARNING > 0 && (
                  <span className="bg-warning ml-2 max-w-min rounded-full px-2 font-bold text-black ">
                    {props.teamstats[team.name].WARNING}
                  </span>
                )}
                {props.teamstats[team.name] && props.teamstats[team.name].FAILURE > 0 && (
                  <span className="bg-failure ml-2 max-w-min rounded-full px-2 font-bold text-black ">
